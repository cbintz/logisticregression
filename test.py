...
import unittest
from sgd import logistic, dot, predict, accuracy, update, submission, extract_features
...
class SGDTest(unittest.TestCase):
    ...
    def test_update(self):
        ...
        point = {'features':[.4,1,3,.01,.1], 'label': 1}
        model2 = model[:]
        update(model2, point, alpha=0.1, lambd=0)
        self.assertAlmostEqual(model2[0], 1.0001628055086358)
        self.assertAlmostEqual(model2[1], 2.0004070137715897)
        self.assertAlmostEqual(model2[2], 1.0012210413147689)
        model2 = model[:]
        update(model2, point, alpha=0.1, lambd=1)
        self.assertAlmostEqual(model2[0], 1.0001628055086358)
        self.assertAlmostEqual(model2[1], 1.8004070137715895)
        self.assertAlmostEqual(model2[2], 0.9012210413147688)
